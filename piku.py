    ...
    with open(filename, "r") as procfile:
        for line_number, line in enumerate(procfile):
            ...
            try:
                ...
                if kind == "cron":
                    ...
                    if res:
                        ...
                        for i in range(len(limits)):
                            if (
                                int(matches[i].replace("*/", "").replace("*", "1"))
                                > limits[i]
                            ):
                                raise ValueError
                ...
                echo(
                    "Warning: misformatted Procfile entry '{}' at line {}".format(
                        line, line_number
                    ),
                    fg="yellow",
                )
    ...
    with open(filename, "r") as settings:
        for line in settings:
            ...
            try:
                ...
                echo(
                    "Error: malformed setting '{}', ignoring file.".format(line),
                    fg="red",
                )
                return {}
    return env
...
def do_deploy(app, deltas={}, newrev=None):
    ...
    if exists(app_path):
        ...
        if newrev:
            call(
                "git reset --hard {}".format(newrev), cwd=app_path, env=env, shell=True
            )
        ...
        if workers and len(workers) > 0:
            ...
            if "preflight" in workers:
                ...
                retval = call(
                    workers["preflight"], cwd=app_path, env=settings, shell=True
                )
                if retval:
                    echo(
                        "-----> Exiting due to preflight command error value: {}".format(
                            retval
                        )
                    )
                    exit(retval)
            ...
            if exists(join(app_path, "requirements.txt")) and found_app("Python"):
                ...
                exists(join(app_path, "Gemfile"))
                and found_app("Ruby Application")
                and check_requirements(["ruby", "gem", "bundle"])
            ):
                settings.update(deploy_ruby(app, deltas))
            elif (
                exists(join(app_path, "package.json"))
                and found_app("Node")
                and (
    ...
    if (
        ...
        if "NGINX_SERVER_NAME" in env:
            ...
            if not exists(cache_path):
                echo(
                    "=====> Cache path {} does not exist, using default {}, be aware of disk usage.".format(
            ...
            if len(cache_prefixes):
                ...
                try:
                    ...
                    echo(
                        "-----> nginx will cache /({}) prefixes up to {} or {} of disk space, with the following timings:".format(
                            cache_prefixes, cache_time_expiry, cache_size
                        )
                    )
                    echo(
                        "-----> nginx will cache content for {}.".format(
                            cache_time_content
                        )
                    )
                    echo(
                        "-----> nginx will cache redirects for {}.".format(
                            cache_time_redirects
                        )
                    )
                    echo(
                        "-----> nginx will cache everything else for {}.".format(
                            cache_time_any
                        )
                    )
                    echo(
                        "-----> nginx will send caching headers asking for {} seconds of public caching.".format(
                            cache_time_control
                        )
                    )
                    ...
                    echo(
                        "Error {} in cache path spec: should be /prefix1:[,/prefix2], ignoring.".format(
                            e
                        )
                    )
                    env["PIKU_INTERNAL_NGINX_CACHE_MAPPINGS"] = ""
...
def spawn_worker(app, kind, command, env, ordinal=1):
    ...
    settings = [
        ...
        (
            "logfile-chown",
            "%s:%s" % (getpwuid(getuid()).pw_name, getgrgid(getgid()).gr_name),
        ),
        ("logfile-chmod", "640"),
    ...
    if "UWSGI_IDLE" in env:
        try:
            ...
            echo(
                "-----> uwsgi will start workers on demand and kill them after {}s of inactivity".format(
                    idle_timeout
                ),
                fg="yellow",
            )
            ...
            echo(
                "Error: malformed setting 'UWSGI_IDLE', ignoring it.".format(), fg="red"
            )
            pass
...
piku.rc = getattr(piku, "result_callback", None) or getattr(
    piku, "resultcallback", None
)
...
def cmd_config(app):
    ...
    if exists(config_file):
        ...
        echo(
            "Warning: app '{}' not deployed, no config found.".format(app), fg="yellow"
        )
